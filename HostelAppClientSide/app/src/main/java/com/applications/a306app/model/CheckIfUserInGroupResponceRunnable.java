...
public class CheckIfUserInGroupResponceRunnable implements  Runnable{
    ...
    public CheckIfUserInGroupResponceRunnable(Socket inputSocket, Handler handler) {
    ...
    public void run() {
        ...
        try {
            while ((inputClient = fromServerReader.readLine()) != null) {
                ...
                if(!resultString.equals(XMLConstants.TAGS_GROUP_VALIDATION_FAIL)) {
                    UsersDB.setGroupForHostUser(resultString);
                    resultString="You are in the "+resultString+ " group!";
                }

                if(messageType== HandleServer.HandleServerResponseConstants.SETGROUPVALIDATIONMESSAGE)
                    bundle.putString(XMLConstants.TAGS_GROUP_VALIDATION,resultString);
