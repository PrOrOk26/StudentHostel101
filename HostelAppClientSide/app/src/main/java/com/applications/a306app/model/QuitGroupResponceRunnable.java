...
public class QuitGroupResponceRunnable implements Runnable{
    ...
    public QuitGroupResponceRunnable(Socket inputSocket, Handler handler) {
    ...
    public void run() {
        ...
        try {
            while ((inputClient = fromServerReader.readLine()) != null) {
                ...
                if(messageType== HandleServer.HandleServerResponseConstants.SETGROUPUSERQUITMESSAGE) {
                        UsersDB.setGroupForHostUser(new String());
                        bundle.putString(XMLConstants.TAGS_GROUP_USER_QUIT,resultString);
                }
